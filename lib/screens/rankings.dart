...
class _Rankings extends State<Rankings> {
  ...
  void rankingAlgorithm(allItems, answers) {
    ...
    for(int i=0; i<allItems.length; i++) {
      if (allItems[i]['category'] == 1 || allItems[i]['category'] == 9 || allItems[i]['category'] == 10 || allItems[i]['category'] == 11 || allItems[i]['category'] == 12) {
        categoryScore = 50;
      }
      else {
      ...
      //TYPE -------------------------------------------------
      List<String> order = answers[8].substring(1,answers[8].length-1).split(', ');;
      ...
      debugPrint(rank.round().toString());
      allItems[i]['rank'] = rank.round().toString();
    }
  }
  ...
  Widget getRanking(allItems) {
    //getData();
    return Container(
      child: ListView(
        children: allItems.map<Widget>((element) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: Center(child: getRanking(allItems)),
